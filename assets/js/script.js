...
showOn.addEventListener('click', function () {
    ...
    h2.innerText = 'Tambah Buku Baru';

    const editNameButton = document.getElementById('bookSubmit');
    editNameButton.innerText = 'Tambah Buku';
});
...
// Penanganan Web Storage
const STORAGE_KEY = 'BOOKSHELF_APPS';
const SAVED_EVENT = 'saved-book';

function storageExist() {
    if (typeof Storage === 'undefined') {
        alert('Maaf, browser Anda tidak mendukung penyimpanan lokal.');
        return false;
    }
    return true;
}
...
function deleteBook(bookId) {
    ...
    const confirmDelete = confirm('Anda yakin ingin menghapus buku ini?');
...
showOn.addEventListener('click', () => {
    ...
    showOn.disabled = true;
});

showOff.addEventListener('click', () => {
    overlay.style.display = 'none';
    showOn.disabled = false;
});
